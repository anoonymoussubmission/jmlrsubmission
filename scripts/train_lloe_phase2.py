...
from lib.lsoe_utils.lsoe import first_phase_soe
...
def parse_args():
    ...
    parser.add_argument('-config', '--config_path', type=str, default='../configs/lloe_phase2/uniform_baseline.json', required=True,
...
def main(args):
    ...
    if hyper_search:
        ...
        # permuted_indices = np.random.permutation(np.array(range(vec_data.shape[0])))
        # test_embedded_indices = permuted_indices[0:100]
        # test_embedded_data = vec_data[test_embedded_indices, :]

        # non_embedded_indices = permuted_indices[100:]
        # non_embedded_points = vec_data[non_embedded_indices, :]
